...
- 各固定ページの本文をハードコード、ブロック、カスタムフィールドのどれで管理するか。
- FAQ、ご利用の流れ、料金、施設・スタッフ情報のデータ構造。
- お知らせのカテゴリー構成、月別絞り込み、ページネーション仕様。
- お問い合わせフォーム、SEO、パンくずに使用するプラグインとテーマ側の責務。
- ナビゲーションを管理画面編集にする範囲と、プライバシーポリシーの現在地表現。
- テーマ同梱画像とメディアライブラリ管理画像の境界。
