...
- テーマ `hidamari-care-asahikawa` 0.11.2、サイト機能プラグイン `hidamari-site-core` 0.6.0、Forminator 1.57.0、SEO SIMPLE PACK 3.7.0で実装済み。
- 固定ページ8件、投稿10件、FAQ13件、利用フロー8件、料金30行、画像、フォーム、SEO、OGP、サイトアイコンを冪等なLocal専用スクリプトで投入済み。
- Localで12 URL・指定9幅の計108状態、サイトマップ20 URL、内部リンク・画像152 URL、Forminatorの確認・デモ送信・非通知・非保存まで再確認済み。本番公開と実メール送信は未実施。
- 0.6.0では、施設担当者を編集者権限とし、ホーム＋指定6固定ページだけを編集可能にした。固定ページの作成・削除、非許可ページ編集、タイトル・スラッグ・公開状態・親子構造・テンプレート変更はサーバー側で拒否または保持する。
- 仮編集者と仮下書きだけを使う68項目の権限テストに合格し、終了後のユーザー・件数一致を確認した。最新版WPvividバックアップは一時MySQL DBと一時WordPressへ実復元し、内部状態22/22、公開12/12 URL、管理者ログインに合格した。
...
`hidamari-site-core` 0.6.0は、施設担当者向け編集者に対して次を実装する。

- 許可固定ページ: `home`、`about-us`、`facilities`、`price`、`faq`、`contact`、`privacy-policy`
- 非許可固定ページ: 投稿一覧に使う`news`。編集者は閲覧できても編集操作はできない
- 全固定ページ: 新規作成・削除、クイック編集・一括編集を禁止
- 許可固定ページ: 本文、導入文、指定画像、アイキャッチを更新可能
- 管理者管理項目: タイトル、スラッグ、公開状態、親子構造、作成者、パスワード、コメント／トラックバック状態、テンプレートを保存時に保持
- 共通施設情報、フォーム、SEO全体設定、テーマ、プラグイン、ユーザーは管理者だけが変更
...
| 共通施設情報 | 「設定 > ひだまり設定」 | 法人名、施設名、サービス表記、住所、施設電話、お問い合わせ電話、各`tel:`用番号、受付時間 | フッター、施設紹介、問い合わせ、構造化された共通表示 |
...
3. 完了: メール送信・DB保存を行わないデモであることを明記

- お名前、メールアドレス、件名、本文、同意は必須にする。
- `autocomplete` は氏名を `name`、メールを `email` とし、プラグインのHTML属性設定または出力フィルターで付与する。
- ポートフォリオ用デモとして、管理者通知と利用者への自動返信は設定しない。
- 個人情報を保持しないため、Forminatorの送信データ保存は無効にする。
- honeypotを有効にし、公開後の機械投稿や負荷に応じてTurnstileまたはレート制限を検討する。
- 入力内容は処理のためWordPressへ渡るが、メール送信・DB保存されないことを入力前と完了時に明記する。
- 実際の問い合わせ窓口へ変更する場合は、送信先、Reply-To、SMTP、保存期間、閲覧権限、Turnstileを別途設計し、秘密情報をGitへ保存しない。
...
11. Forminatorの3段階デモフォーム、通知なし、honeypot、DB保存なしを設定し、問い合わせページへ埋め込む。
...
- Forminatorの入力・確認・戻る・送信・デモ完了表示、通知なし、DB保存なし、honeypotを確認できる。
