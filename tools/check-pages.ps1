...
try {
  ...
  if (Test-Path -LiteralPath $profileRoot) {
    for ($cleanupAttempt = 0; $cleanupAttempt -lt 40; $cleanupAttempt++) {
      try {
        Remove-Item -LiteralPath $profileRoot -Recurse -Force -ErrorAction Stop
        break
      } catch {
        if ($cleanupAttempt -eq 39) {
          Write-Warning ("Could not remove Chrome check profile {0}: {1}" -f $profileRoot, $_.Exception.Message)
        } else {
          Start-Sleep -Milliseconds 250
        }
      }
