...
- 使用画像の出典と利用条件はユーザーが手動確認し、ポートフォリオサイトでの公開に問題がないことを確認したため、公開前確認の画像権利チェックを完了にした。
- 現行デモの電話番号を「掲載なし（デモ）」へ変更して電話リンクをなくし、住所を `北海道旭川市（架空設定）`、送迎範囲を架空設定と明記した。お知らせ一覧・詳細にはサンプル記事の案内を追加し、記事3件、見学フロー、プライバシーポリシーの電話受付に関する矛盾表現も修正した。
- SEO SIMPLE PACKの固定ページ8件、投稿10件、カテゴリー、日付アーカイブの説明を、検索結果でもポートフォリオ用の架空サイトと分かる内容へ更新した。配信中の全20 URLでdescriptionに「ポートフォリオ用」が含まれることを確認した。テーマを0.11.2へ更新し、WordPress版CSSを再生成した。
- LocalへTOP、施設紹介、FAQ、投稿、プライバシーポリシー、SEOの投入処理を再実行し、固定ページ8件、投稿10件、FAQ13件、利用フロー8件などの件数を維持した。サイトマップ掲載URL20件で旧伏せ字電話番号・旧住所・電話リンクが0件、共通デモ表示が各1件であることを確認した。
- PC 1600px・SP 390pxでTOP、お問い合わせ、お知らせ一覧、投稿詳細を確認し、追加文言の折り返し、横スクロールなし、JavaScriptエラーなしを確認した。Chromeプラグインは既知の読み込みエラーだったため、Playwright経由のローカルChromeを代替利用した。変更したPHPは構文エラーなし、CSSビルドと `git diff --check` も成功した。
- 公開前確認の項目1「仮情報、実在情報、画像権利」は全完了条件を満たした。次は項目2「PC・スマートフォンの表示」へ進む。
- 項目1完了後のWPvivid統合バックアップを作成し、成功・削除防止ロック有効を確認した。最新版は `handoff/260819-latest/hidamari-care-asahikawa.local_wpvivid-154982c0c64de_2026-08-19-02-46_backup_all.zip`、46,581,102 bytes、SHA-256は `3FD1E8CC500BCF1A9B9AE5FE84F0888949463E206A6CEF086A319ED44B388721`。元ファイルとのハッシュ一致、6パッケージ構成、テーマ0.11.2、電話受付なし表示、サンプル記事案内を確認した。
