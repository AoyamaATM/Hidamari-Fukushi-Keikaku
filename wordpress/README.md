...
現行デモでは電話番号を掲載せず、電話リンクが未設定の場合は「掲載なし（デモ）」と表示する。住所と送迎範囲には架空設定であることを明記し、お知らせ一覧・詳細にはサンプル記事の案内を表示する。実在環境へ移行するときは「設定 > ひだまり設定」で承認済みの住所・電話番号・発信用番号へ差し替え、正式記事とサービス提供範囲を投入する。
